...
socket.emit('register new socket', location.host);
...
socket.on('get players points', (data) => {
  ...
  playerLeft = points.me;
  playerRight = points.adv;
...
socket.on('move ball', function(dt){
  ...
  if ((vx < 0 && cx <= 0) || (vx > 0 && cx >= width)) {
    ...
    if(vx < 0) 
      socket.emit('up point', 2);
    else if(vx >= 0) 
      socket.emit('up point', 1);
...
socket.on('move paddle 1', function(direction){
  console.log("direction player 1: " + direction);
  paddleLeftDirection = direction;
});

socket.on('move paddle 2', function(direction){
  console.log("direction player 2: " + direction);
